...
# This script characterises patients who switch cluster between the primary
# SNF solution and each sensitivity analysis. It compares switchers with
# non-switchers using clinical and metabolomic variables and produces summary
# tables and plots for each sensitivity branch.
#
# Fixed settings:
# - Clinical comparisons use Wilcoxon and t-tests.
# - Metabolite comparisons use Wilcoxon and t-tests.
# - P-values are adjusted using the Benjamini-Hochberg (BH) method.
# - Significance threshold = 0.05.
#
# Before running:
# - Set `project_root` to the local project directory.
# - Processed clinical and metabolomics data must already exist.
# - Switcher tables must already have been generated by `05_cluster_switchers.R`.
#
# Outputs are saved under `paths$posthoc/switcher_characterisation/`, including
# clinical and metabolomic comparisons, branch summaries, and plots.
