...
# -------------------------------------------------------------------
snf_dir <- file.path(paths$snf, "main_four_network")
fused_file <- file.path(snf_dir, "primary_four_network_fused_network.rds")
sample_id_file <- file.path(snf_dir, "primary_four_network_sample_ids.rds")

if (!file.exists(fused_file)) {
  stop("Missing fused network file: ", fused_file)
}
if (!file.exists(sample_id_file)) {
  stop("Missing sample ID file: ", sample_id_file)
}

out_dir <- file.path(snf_dir, "cluster_solution")
...
# -------------------------------------------------------------------
fused_network <- read_square_matrix(fused_file, "Fused network")
sample_ids <- readRDS(sample_id_file)

if (is.null(sample_ids) || length(sample_ids) == 0) {
  stop("sample_ids file is empty or invalid: ", sample_id_file)
}
...
plot(
  ...
  ylab = "Eigengap",
  main = "Eigengap candidates for the primary fused network"
)
