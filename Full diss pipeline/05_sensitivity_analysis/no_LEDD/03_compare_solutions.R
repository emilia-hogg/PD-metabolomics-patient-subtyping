...
branch_dir <- file.path(project_root, "05_sensitivity_analysis", "outputs", "no_LEDD")
...
branch_results_file <- file.path(branch_dir, "no_ledd_cluster_results.rds")
...
if (!all(c("fused_network", "sample_ids", "cluster_labels") %in% names(branch_obj))) {
  stop("no_ledd_cluster_results.rds must contain fused_network, sample_ids, and cluster_labels.")
}
...
prefix <- "no_ledd_vs_main_four_network"
...
message("no_LEDD comparison complete.")
