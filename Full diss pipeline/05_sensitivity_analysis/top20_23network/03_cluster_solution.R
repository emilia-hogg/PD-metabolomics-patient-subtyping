...
branch_dir <- file.path(paths$sensitivity, "top20_23network")
out_dir <- file.path(branch_dir, "cluster_solution")
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

snf_results_file <- file.path(branch_dir, "top20_23network_snf_results.rds")
...
if (!all(c("fused_network", "sample_ids") %in% names(snf_obj))) {
  stop("top20_23network_snf_results.rds must contain fused_network and sample_ids.")
}
...
message("Top-20 23-network clustering complete.")
